...
const SectionTitle = ({ eyebrow, title, description, center = true, light = false }) => {
  return (
    ...
    >
      {eyebrow && (
        <span
          className={`inline-flex items-center gap-2 text-xs md:text-sm font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-4 ${
            light ? 'bg-accent/20 text-accent' : 'bg-primary/10 text-primary'
          }`}
        >
      ...
      <h2 className={`text-3xl md:text-4xl lg:text-[2.6rem] font-bold leading-tight ${light ? 'text-white' : 'text-ink'}`}>
        {title}
      </h2>
      {description && (
        <p className={`mt-4 text-base md:text-lg ${light ? 'text-slate-100' : 'text-muted'}`}>{description}</p>
      )}
    </motion.div>
  );
};
